...
Proof.
  intros n.
  induction n as [| n' IHn'].
  - intros m eq1. induction m as [| m' IHm'].
    + reflexivity. 
    + discriminate eq1.
  - intros m eq2. induction m as[| m' IHm'].
    + discriminate eq2.
...
Proof.
  ...
  - destruct n as [| n'].
    ...
    + simpl. intros eq2. apply IHl'. injection eq2 as goal.
      apply goal.
Qed.
(** [] *)
...
Proof.
  intros X Y l.
  induction l as [| h l' IHl'].
  - intros l1 l2 eq. injection eq as H.
    rewrite <- H. rewrite <- H0. reflexivity.
  - destruct h as (x, y).
    destruct l1 as [| h1 l1'].
    * intros l2 eq1. simpl in eq1.
      destruct (split l') in eq1.
      discriminate eq1.
    * intros l2 eq2. simpl.
      destruct l2 as [| h2 l2'].
      + simpl in eq2. destruct (split l').
        discriminate eq2.
      + simpl in eq2. destruct (split l').
        injection eq2 as H1 H2 H3 H4.
...
Proof.
  intros f b.
  destruct b eqn:Heqb.
  - destruct (f true) eqn:Heqf.
    + rewrite Heqf. rewrite Heqf.
      reflexivity.
    + destruct (f false) eqn:Heqf1.
      * apply Heqf.
      * apply Heqf1.
  - destruct (f false) eqn:Heqf.
    + destruct (f true) eqn:Heqf1.
      * apply Heqf1.
      * apply Heqf.
    + rewrite Heqf. apply Heqf.
Qed.
(** [] *)
...
Proof.
  ...
  - induction m as [| m' IHm'].
    + reflexivity.
    + rewrite S_neqb_0. rewrite zero_neqb_S.
      reflexivity.
  - induction m as [| m' IHm'].
    + rewrite S_neqb_0. rewrite zero_neqb_S.
      reflexivity.
...
(* 
exits x P(x) = neg (forall x neg P(x))
forall x P(x) = neg (exits x neg P(x))
*)
