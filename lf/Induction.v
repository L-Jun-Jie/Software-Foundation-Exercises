...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    - First, suppose [n = 0]. We must show that
      0 =? 0 = true.
      This follws directly from the definition of [=?].
    - Next, suppose [n = S n'], where
      n' =? n' = true.
      We must show that
      (S n') =? (S n') = true.
...
Proof.
  ...
  - rewrite mul_0_r. 
    ...
    rewrite mul_0_r. 
    reflexivity.
  - assert (H1: (n + m) * S p' = (n + m) + (n + m) * p').
    { rewrite <- mult_n_Sm. rewrite add_comm. reflexivity. }
    assert (H2: n * S p' = n + n * p').
    { rewrite <- mult_n_Sm. rewrite add_comm. reflexivity. }
    assert (H3: m * S p' = m + m * p').
    { rewrite <- mult_n_Sm. rewrite add_comm. reflexivity. }
    rewrite H1. rewrite H2. rewrite H3.
    rewrite add_assoc.
    assert (H4: n * p' + m = m + n * p').
    { rewrite add_comm. reflexivity. }
    assert (H5: n + n * p' + m = n + (n * p' + m)).
    { rewrite add_assoc. reflexivity. }
    rewrite -> H5.
    rewrite -> H4.
    assert (H6: n + (m + n * p') = n + m + n * p').
    { rewrite add_assoc. reflexivity. }
    rewrite -> H6.
    rewrite IHp'.
    rewrite add_assoc.
...
Proof. intros b.
  ...
  - simpl. rewrite add_0_r. rewrite <- IHb'.
    destruct (bin_to_nat b') as [| n']. (* destruct nat !!! *)
    + simpl. reflexivity.
